...
from physicsnemo.sym.geometry.parameterization import OrderedParameterization # <== for causal loss ==
...
from physicsnemo.sym.loss.loss import CausalLossNorm # <== causal loss ==
...
def run(cfg: PhysicsNeMoConfig) -> None:
    ...
    time_range = OrderedParameterization({t_symbol: (0, _t_f)}, key=t_symbol) 
    # geo
    geo_1D = Line1D(
    ...
    interior = PointwiseInteriorConstraint(
        ...
        loss = CausalLossNorm(
            eps = 1.0,
            n_chunks = 16
        ), # <=== from here, additions for causal loss
        fixed_dataset = False,
        shuffle = False,
    )
